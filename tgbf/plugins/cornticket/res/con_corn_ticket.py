...
def init():
    ...
    last_won_corn.set(0)
    total_won_tau.set(0)
    total_won_corn.set(0)
    last_reserve_corn.set(0)
    total_reserve_corn.set(0)
...
def buy_ticket():
    ...
    corn_price = ForeignHash(
        foreign_contract='con_rocketswap_official_v1_1',
        foreign_name='prices')

    corn_amount = tau_amount.get() / corn_price["con_bitcorn"]
    ...
    corn_balance.set(corn_balance.get() + corn_amount)
...
def draw_winner():
    ...
    last_reserve_corn.set(corn_balance.get() / 100 * (dev_share.get() + endo_share.get()))
    total_reserve_corn.set(total_reserve_corn.get() + last_reserve_corn.get())

    last_won_tau.set(tau_balance.get() - dev_amount_tau - endo_amount_tau)
    last_won_corn.set(corn_balance.get() - last_reserve_corn.get())

    total_won_tau.set(total_won_tau.get() + last_won_tau.get())
    total_won_corn.set(total_won_corn.get() + last_won_corn.get())
    ...
    corn.transfer(
        amount=last_won_corn.get(),
        to=winner)

    # Transfer GOLD from contract to GOLD Reserve address
    corn.transfer(
        amount=last_reserve_corn.get(),
        to="96dae3b6213fb80eac7c6f4fa0fd26f34022741c56773107b20199cb43f5ed62")
