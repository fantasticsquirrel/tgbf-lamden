...
class Goldflip(TGBFPlugin):
    ...
    def load(self):
        self.add_handler(CommandHandler(
            self.name,
            self.goldflip_callback,
            run_async=True))

    @TGBFPlugin.blacklist
    @TGBFPlugin.send_typing
    def goldflip_callback(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            logging.error(f"Error approving goldflip contract: {e}")
        ...
        try:
            ...
            logging.error(f"Error calling goldflip contract: {e}")
            message.edit_text(f"{emo.ERROR} {e}")
            return

        logging.info(f"Executed goldflip contract: {flip}")

        if "error" in flip:
            logging.error(f"Goldflip contract returned error: {flip['error']}")
        ...
        if not success:
            logging.error(f"Goldflip transaction not successful: {result}")
        ...
        if result["result"].upper() == "'YOU WON'":
            logging.info(f"User WON <code>{amount}</code> {self.TOKEN_SYMBOL}")
