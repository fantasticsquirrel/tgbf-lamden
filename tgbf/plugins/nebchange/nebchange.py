...
class Nebchange(TGBFPlugin):
    ...
    def check_price_change(self, context: CallbackContext):
        ...
        try:
            ...
            logging.info(f"NebChange exception attempting to query last trades: {e}")
            return
        if not maxtrades or not maxtrades["data"]:
            logging.info("NebChange did not find any last trades? Exiting...")
            return
        ...
        try:
            ...
            logging.info(f"NebChange exception attempting to query trades: {e}")
            return
        if not trades or not trades["data"]:
            logging.info("NebChange did not find any trades? Exiting...")
            return
