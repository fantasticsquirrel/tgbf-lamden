    ...
    def price_callback(self, update: Update, context: CallbackContext):
        ...
        if exchange:
            ...
            for d in data["tickers"]:
                if exchange in d["market"]["name"].lower():
                    usd = d["converted_last"]["usd"]
                    btc = d["converted_last"]["btc"]
                    eth = d["converted_last"]["eth"]
                    ...
                    ex_name = d["market"]["name"]
                    msg = f"{name} ({symbol}) on {ex_name}\n\n" \
                          f"USD {p_usd}\n" \
                          f"BTC {p_btc}\n" \
                          f"ETH {p_eth}\n\n"

            if not msg:
                msg = f"{emo.ERROR} Unknown exchange name"
