...
class Rtaubuyback(TGBFPlugin):

    def load(self):
        self.add_handler(CommandHandler(
            self.handle,
            self.buyback_callback,
            run_async=True))

    @TGBFPlugin.send_typing
    def buyback_callback(self, update: Update, context: CallbackContext):
        ...
        update.message.reply_text(
            f'<code>{tau_pool:,}</code> TAU in Buy-Back-Pool',
